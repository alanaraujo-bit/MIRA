...
export default function ProductApp({ user }: { user: { displayName: string; email: string } }) {
  ...
  const fetchWorkspaceData = useCallback(async (
    ...
    const params = new URLSearchParams({ workspaceId, query, status });
    try {
  ...
  useEffect(() => {
    ...
    const searchTimer = window.setTimeout(
      () => void fetchWorkspaceData(workspace.id, search, statusFilter),
      240,
    );
    return () => window.clearTimeout(searchTimer);
  }, [fetchWorkspaceData, search, state, statusFilter, workspace]);
  ...
  async function patchLink(item: LinkItem, changes: Partial<Pick<LinkItem, "title" | "slug" | "status">> & { destinationUrl?: string }) {
    setMutating(item.id);
    setNotice(null);
    try {
      ...
      if (workspace) await fetchWorkspaceData(workspace.id, search, statusFilter);
    } catch (error) {
          ...
          <section className="links-panel" id="links">
            ...
            {collectionState === "error" ? <div className="links-empty error"><span>!</span><h3>Não foi possível atualizar a biblioteca.</h3><button className="button quiet" type="button" onClick={() => void fetchWorkspaceData(workspace.id, search, statusFilter)}>Tentar novamente</button></div> : links.length === 0 ? <div className="links-empty"><span>{search || statusFilter !== "all" ? "⌕" : "↗"}</span><h3>{search || statusFilter !== "all" ? "Nenhum link corresponde aos filtros." : "Seu primeiro link começa aqui."}</h3><p>{search || statusFilter !== "all" ? "Ajuste a busca ou volte para Todos." : "Preencha o formulário acima. Nenhum dado de demonstração está ocupando este espaço."}</p></div> : <div className={`links-table ${collectionState === "loading" ? "is-loading" : ""}`} role="table" aria-label="Links do Workspace" aria-busy={collectionState === "loading"}>
              ...
              {links.map((item) => <div className="link-row" key={item.id} role="row">
                <div><span className={`link-status ${item.status}`} aria-label={item.status === "active" ? "Ativo" : item.status === "archived" ? "Arquivado" : "Bloqueado"} /><strong>{item.title}</strong><small>/go/{item.slug} · {item.status === "active" ? "ativo" : item.status === "archived" ? "arquivado" : "bloqueado"}</small></div>
