...
export default function CampaignInspector({ campaignId, user }: { campaignId: string; user: { displayName: string; email: string } }) {
  ...
  return <div className="product-shell"><a className="skip-link" href="#campaign-inspector-main">Ir para o conteúdo</a>
    <aside className="product-sidebar"><Link className="brand" href="/" aria-label="Mira Roadmap"><span className="brand-mark" aria-hidden="true"><i /><i /></span><span>Mira</span></Link><div className="workspace-switcher"><span>Workspace</span><strong>{workspace?.name ?? "Carregando…"}</strong><small>{workspace?.role ?? ""}</small></div><nav aria-label="Produto"><Link href="/product"><span aria-hidden="true">⌂</span>Visão geral</Link><Link href="/product#links"><span aria-hidden="true">↗</span>Links</Link><span aria-disabled="true"><span aria-hidden="true">◌</span>Analytics <small>em breve</small></span><Link className="selected" href="/product/campaigns"><span aria-hidden="true">◇</span>Campaigns</Link></nav><Link className="roadmap-return" href="/">Roadmap Live <span>↗</span></Link></aside>
