    ...
    <main id="campaign-main" className="product-main">
      ...
      {state === "ready" && workspace && <div className="product-content campaigns-content">
        ...
        <section className="links-panel campaigns-panel"><header><div><span className="eyebrow">Portfólio</span><h2>Campaigns</h2></div><span>{campaigns.length} {campaigns.length === 1 ? "item" : "itens"}</span></header>
          {campaigns.length === 0 ? <div className="links-empty"><span>◇</span><h3>Organize antes de distribuir.</h3><p>Crie uma Campaign e associe seus próximos Links a ela.</p></div> : <div className="campaign-list">{campaigns.map((campaign) => <article key={campaign.id}><div className="campaign-main"><span className={`campaign-status ${campaign.status}`}>{campaign.status === "active" ? "Ativa" : campaign.status === "planning" ? "Planejamento" : "Encerrada"}</span><h3>{campaign.name}</h3><p>{campaign.objective ?? "Sem objetivo documentado."}</p></div><dl><div><dt>Links</dt><dd>{campaign.links}</dd></div><div><dt>Cliques</dt><dd>{campaign.clicks.toLocaleString("pt-BR")}</dd></div></dl><button type="button" disabled={mutating === campaign.id} onClick={() => void changeStatus(campaign, campaign.status === "ended" ? "active" : "ended")}>{mutating === campaign.id ? "Atualizando…" : campaign.status === "ended" ? "Reabrir" : "Encerrar"}</button></article>)}</div>}
        </section>
      </div>}
    </main>
  </div>;
