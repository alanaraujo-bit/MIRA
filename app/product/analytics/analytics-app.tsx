...
type Report = {
  ...
  metrics: { clicks: Comparison; sessions: Comparison; sessionCoverage: number; clicksPerSession: number;
    activeLinks: number; linksWithTraffic: number; automatedClicks: number; automatedShare: number; lastEventAt: number | null };
  series: SeriesPoint[]; sources: Breakdown[]; devices: Breakdown[]; countries: Breakdown[];
  languages: Breakdown[]; operatingSystems: Breakdown[]; browsers: Breakdown[];
  ...
  return <div className="product-shell">
    ...
    <main id="analytics-main" className="product-main">
      ...
      {state === "ready" && report && <div className={`product-content analytics-content ${refreshing ? "is-refreshing" : ""}`}>
        ...
        <section className="analytics-metrics" aria-label="Resumo de performance"><article className="primary"><span>Cliques</span><strong>{report.metrics.clicks.current.toLocaleString("pt-BR")}</strong><Delta value={report.metrics.clicks} /></article><article><span>Sessões observadas</span><strong>{report.metrics.sessions.current.toLocaleString("pt-BR")}</strong><Delta value={report.metrics.sessions} /><small>{report.metrics.sessionCoverage}% de cobertura</small></article><article><span>Cliques por sessão</span><strong>{report.metrics.clicksPerSession.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}</strong><small>somente eventos com sessão</small></article><article><span>Links com tráfego</span><strong>{report.metrics.linksWithTraffic}</strong><small>de {report.metrics.activeLinks} ativos</small></article><article><span>Automação conhecida</span><strong>{report.metrics.automatedShare}%</strong><small>{report.metrics.automatedClicks} eventos classificados</small></article><article><span>Último evento</span><strong className="metric-date">{report.metrics.lastEventAt ? dateTime.format(report.metrics.lastEventAt) : "—"}</strong><small>{report.metrics.lastEventAt ? "persistência confirmada" : "aguardando tráfego"}</small></article></section>
        ...
        <div className="analytics-grid"><section className="analytics-panel"><header><div><span className="eyebrow">Aquisição</span><h2>Origens</h2></div><span>{report.sources.length} identificadas</span></header><BreakdownRows items={report.sources} empty="Os próximos cliques revelarão as origens disponíveis." /></section><section className="analytics-panel"><header><div><span className="eyebrow">Audiência</span><h2>Países</h2></div><span>localização aproximada</span></header><BreakdownRows items={report.countries} empty="Ainda não existe contexto geográfico disponível." /></section></div>
        <div className="analytics-grid"><section className="analytics-panel"><header><div><span className="eyebrow">Contexto</span><h2>Dispositivos</h2></div><span>classificação minimizada</span></header><BreakdownRows items={report.devices} empty="Ainda não existe amostra de dispositivo." /></section><section className="analytics-panel"><header><div><span className="eyebrow">Tecnologia</span><h2>Sistemas</h2></div><span>sem user-agent bruto</span></header><BreakdownRows items={report.operatingSystems} empty="Ainda não existe amostra de sistema operacional." /></section></div>
