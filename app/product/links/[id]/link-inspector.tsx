...
type Analytics = { link: LinkItem; range: { days: number; startsAt: number; endsAt: number }; clicks: Comparison; series: SeriesPoint[];
  ...
  return <div className="product-shell">
    ...
    <main id="link-inspector-main" className="product-main">
      ...
      {state === "ready" && analytics && link && <div className={`product-content inspector-content link-inspector-content ${refreshing ? "is-refreshing" : ""}`}>
        ...
        <section className="link-inspector-metrics"><article><span>Cliques · {days} dias</span><strong>{analytics.clicks.current.toLocaleString("pt-BR")}</strong><Delta value={analytics.clicks} /></article><article><span>Total histórico</span><strong>{link.clicks.toLocaleString("pt-BR")}</strong><small>eventos persistidos</small></article><article><span>Última alteração</span><strong className="metric-date">{dateTime.format(link.updated_at)}</strong><small>controle de concorrência ativo</small></article></section>
