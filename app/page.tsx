      ...
      <main id="main">
        ...
        <section className="evidence-grid" id="releases">
          ...
          <div className="evidence-columns">
            <article className="evidence-card"><header><span>Release atual</span><strong>{project.release}</strong></header><h3>Preview público real</h3><p>Conta própria, Workspaces, Links, Campaigns, redirects, eventos, analytics, QR e domínios operam sobre persistência Railway; gates restantes continuam explícitos.</p><a className="release-link" href={project.liveUrl} target="_blank" rel="noreferrer">Abrir release <span aria-hidden="true">↗</span></a><dl><div><dt>Ambiente</dt><dd>{project.environment}</dd></div><div><dt>Origem</dt><dd>{project.commit}</dd></div><div><dt>Saúde</dt><dd className="status-inline active">Deploy em validação</dd></div></dl></article>
