...
A release privada publicada `0.2.0-alpha.3` amplia o M2 com onboarding de domínio, verificação TXT real, estados de DNS/SSL e associação de Links à identidade de marca. M1 e M2 continuam ativos enquanto inspeção visual, ativação pública de redirects, um ownership positivo em DNS controlado e execução remota do pipeline forem gates abertos; o Roadmap não os apresenta como concluídos.
